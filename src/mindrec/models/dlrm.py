...
def make_mlp(in_dim: int, layer_dims: list[int], dropout: float=0.0, last_activation: bool=True) -> nn.Sequential:
    layers=[]
    d=in_dim
...
class DLRMStudent(nn.Module):
    def __init__(
        ...
        self.bottom=make_mlp(dense_dim, bottom_mlp, dropout=dropout, last_activation=True)
    ...
    def forward(
        ...
        inter_vec=torch.cat(inter, dim=1) if inter else torch.zeros((xd.size(0), 0), device=xd.device)
