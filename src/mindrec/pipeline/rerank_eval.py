...
from mindrec.metrics.fairness import catalog_target, exposure_from_ranking, gini, kl_divergence, l1_distance, normalize_dist, uniform_target
from mindrec.metrics.ranking import ndcg_at_k, recall_at_k, ndcg_from_order, recall_from_order
...
def run_rerank_eval(cfg: dict[str, Any]) -> None:
    ...
    with torch.no_grad():
        for _,r in tqdm(impr.iterrows(), total=len(impr), desc="Rerank eval"):
            ...
            base_cats=[news_meta.get(nid).cat_idx if nid in news_meta else 0 for nid in base_ids]
            ...
            tgt=uniform_target(base_cats) if fairness_cfg.get("category_target", "catalog")=="uniform" else catalog_target(base_cats)
            ...
            new_exp=sum(float(wi) for wi,flag in zip(w.tolist(), is_new_base) if flag==1)
            ...
            rr_ndcg.append(ndcg_at_k(labels, scores, k_out))  # same definition vs all candidates
            rr_recall.append(recall_at_k(labels, scores, k_out))

            rr_cats=[news_meta.get(nid).cat_idx if nid in news_meta else 0 for nid in rr_ids]
            ...
            tgt2=uniform_target(rr_cats) if fairness_cfg.get("category_target", "catalog")=="uniform" else catalog_target(rr_cats)
            ...
            new_exp2=sum(float(wi) for wi,flag in zip(w.tolist(), is_new_rr) if flag==1)
