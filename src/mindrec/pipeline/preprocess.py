...
def build_pairs(
    ...
    set_seed(seed)
    news_lookup=news_idx_df.set_index("news_id")[["news_idx","cat_idx","subcat_idx"]].to_dict(orient="index")
    ...
    for _,r in tqdm(beh.iterrows(), total=len(beh), desc="Build pairs"):
        ...
        for pi in pos:
            pos_id=cand_ids[pi]
            neg_idx=np.random.choice(neg, size=min(neg_per_pos, len(neg)), replace=False)
...
def run_preprocess(cfg: dict[str, Any]) -> None:
    ...
    news_all=pd.concat([news_train, news_dev], axis=0).drop_duplicates("news_id").reset_index(drop=True)
    ...
    pairs_train=build_pairs(
        ...
        min_item_train_clicks_for_warm=int(cfg["data"]["min_item_train_clicks_for_warm"]),
        neg_per_pos=4,
        seed=seed,
    )
