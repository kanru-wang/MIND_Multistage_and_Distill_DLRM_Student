...
def build_news_meta(news_df) -> dict[str, NewsMeta]:
    ...
    for _,r in news_df.iterrows():
        ent=_parse_entities(r.get("title_entities", "")) | _parse_entities(r.get("abstract_entities", ""))
...
def greedy_rerank(
    ...
    if novelty_sim=="teacher_cosine":
        idx=[int(news_meta.get(nid, NewsMeta(0,0,set())).cat_idx) for nid in pool]  # placeholder
    ...
    def novelty(i: int) -> float:
        ...
        if novelty_sim=="category":
            ...
            sims=[1.0 if ci==news_meta.get(pool[j], NewsMeta(0,0,set())).cat_idx else 0.0 for j in chosen]
            return -max(sims)
        # entity_jaccard
        ei=news_meta.get(pool[i], NewsMeta(0,0,set())).ent
        sims=[jaccard(ei, news_meta.get(pool[j], NewsMeta(0,0,set())).ent) for j in chosen]
        return -max(sims)
    ...
    def fairness_penalty(ranked_ids: list[str]) -> float:
        ...
        if floor > 0.0:
            is_new=[int(pool_is_new[pool.index(nid)]) if nid in pool else 0 for nid in ranked_ids]
            new_exp=float(sum(wi for wi,flag in zip(w.tolist(), is_new) if flag==1))
    ...
    for _ in range(min(k_out, len(pool))):
        ...
        for i,nid in enumerate(pool):
            ...
            val=relevance_weight * rel + novelty_weight * novelty(i) + coverage_weight * coverage(i)

            if fairness_cfg.get("enabled", False) and chosen:
                trial=chosen + [nid]
                val -= float(fairness_cfg.get("penalty_weight", 0.5)) * fairness_penalty(trial)
